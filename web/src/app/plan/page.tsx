            ...
            : !isSignedIn ? <ApiState error={new ApiError('unauthorized', 'Please sign in to view your plan.', 401)} />
                : error ? <ApiState error={error} onRetry={() => window.location.reload()} />
                    : plan && <div className="space-y-6">
                        <header><p className="text-[11px] font-semibold uppercase tracking-widest text-glow">Automatic, explainable study plan</p><h1 className="font-display mt-1 text-3xl font-normal text-cream">Your next five actions</h1><p className="mt-2 text-sm text-cream/50">Built from server-owned attempts, recency, independence, context variety, and review timing.</p></header>
