...
function Section({ id, title, description, children }: { id: string; title: string; description: string; children: ReactNode }) {
  return <section id={id} className="scroll-mt-40 border-t border-line py-7 first:border-t-0 first:pt-0 sm:py-9 lg:scroll-mt-28"><div className="grid gap-5 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-10"><div><h2 className="font-display text-2xl text-ivory sm:text-3xl">{title}</h2><p className="mt-2 text-sm leading-6 text-stone">{description}</p></div><div className="min-w-0">{children}</div></div></section>
}
...
export default function ProfilePage() {
  ...
  return <AppShell><div className="mx-auto max-w-6xl pb-8">
    <header className="mb-6 pb-3 sm:mb-8"><p className="text-[11px] font-semibold uppercase tracking-[.18em] text-ember-soft">Your Ecla</p><h1 className="font-display mt-3 text-4xl leading-none text-ivory sm:text-6xl">Profile &amp; settings</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-stone sm:text-base">Manage your identity, learning rhythm, appearance, privacy, and account security.</p></header>
    <nav aria-label="Settings sections" className="no-scrollbar sticky top-14 z-20 -mx-4 mb-8 flex gap-1 overflow-x-auto border-y border-line bg-obsidian/95 px-4 py-2 backdrop-blur-xl sm:top-16 sm:-mx-6 sm:px-6 lg:mx-0 lg:rounded-control lg:border lg:px-2">
      {[['profile', 'Profile'], ['appearance', 'Appearance'], ['learning', 'Learning'], ['privacy', 'Privacy'], ['account', 'Account']].map(([id, label]) => <a key={id} href={`#${id}`} className="ecla-control inline-flex min-h-10 shrink-0 items-center rounded-control px-3 text-xs font-medium text-stone hover:bg-surface hover:text-ivory">{label}</a>)}
    </nav>
