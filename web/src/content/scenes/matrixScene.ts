import { record } from '@/lib/jsonBoundary'
...
export function matrixSceneFor(code: string, lesson: LessonPayload, mode?: string): SceneSpec | undefined {
 ...
 */
export function sceneMatrixEntry(code: string, lesson: LessonPayload) {
    ...
    const assessment = record(eng?.assessment)
    ...
    return {
        ...
        characters: dir.cast,
        targetLanguage: {
            patterns: (eng?.languageTargets?.patterns ?? []).slice(0, 6),
            vocabulary: (eng?.languageTargets?.vocabulary ?? []).slice(0, 8),
        },
        input: record(acts('ENCOUNTER', 'listening')?.input).utterances ?? [],
        production: acts('PRODUCE', 'guided_speaking')?.prompt ?? null,
        interaction: record(acts('INTERACT', 'guided_interaction')?.input).opening ?? null,
        ...
        revisit: record(assessment.retention).scheduleDays ?? [1, 2, 4, 7, 14, 30],
    }
}