  ...
  return (
    <ol className="grid gap-px overflow-hidden rounded-[24px] border border-white/[0.08] bg-white/[0.08] sm:grid-cols-3 lg:grid-cols-9">
      {stages.map(({ label, detail, Icon }, index) => (
        <li key={label} className="min-w-0 bg-ink px-4 py-5 sm:px-5 lg:px-3 lg:py-6">
          <div className="flex items-center justify-between gap-3 lg:block">
            <div className="flex items-center gap-3 lg:block">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-ember text-[11px] font-black text-obsidian lg:mb-4 lg:h-7 lg:w-7">
                {index + 1}
              </span>
