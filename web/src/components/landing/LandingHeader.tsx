    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-obsidian/[92%] backdrop-blur-xl supports-[backdrop-filter]:bg-obsidian/80">
      <div className="relative mx-auto flex h-16 w-full max-w-[1240px] items-center px-5 sm:h-[72px] sm:px-8 lg:px-10">
        ...
        <nav aria-label="Primary navigation" className="mx-auto hidden items-center gap-7 lg:flex">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm font-bold text-ivory/50 transition-colors hover:text-ivory focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember/70"
            >
              {item.label}
            </Link>
          ))}
        ...
        <button
          ...
          className="ml-auto grid h-10 w-10 place-items-center rounded-xl border border-white/[0.08] text-ivory/70 transition hover:border-white/20 hover:text-ivory focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember/70 lg:hidden"
        >
