...
export function RealFirefly({ size = 48, className = '', style }: { size?: number; className?: string; style?: CSSProperties }) {
    const uid = useId().replace(/[^a-zA-Z0-9]/g, '')
    return (
        <svg width={size} height={size * 1.16} viewBox="-60 -70 120 140" className={className} style={style} aria-hidden="true">
            <FireflyArt p={uid} />
        </svg>
    )
}
...
export function Logo({ className = 'text-3xl', fireflySize = 40, style }: { className?: string; fireflySize?: number; style?: CSSProperties }) {
    return (
        <span className={`relative inline-flex font-display font-semibold tracking-tight text-cream ${className}`} style={style}>
            <span>Ec</span>
...
export function LogoMark({ size = 48, className, style }: { size?: number; className?: string; style?: CSSProperties }) {
    const uid = useId().replace(/[^a-zA-Z0-9]/g, '')
    return (
        <svg width={size} height={size} viewBox="0 0 512 512" className={className} style={style} aria-hidden="true">
            <defs>
