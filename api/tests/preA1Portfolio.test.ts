...
test('all 60 reconciled Pre-A1 competencies have explicit complete portfolios without duplicate contexts', () => {
    const report = validatePreA1Portfolio(PRE_A1_PORTFOLIO, PRE_A1_CODES)
    assert.equal(PRE_A1_PORTFOLIO.length, 60)
    assert.equal(PRE_A1_PORTFOLIO.reduce((sum, item) => sum + item.contexts.length, 0), 180)
...
test('portfolio stays unpublished until independent cultural and native-speaker reviews exist', () => {
    ...
    assert.equal(report.reviewBlockers.length, 120)
})
...
test('every authored context compiles into a versioned canonical scene draft', () => {
    ...
    assert.equal(new Set(sources.map(source => source.slug)).size, 180)
    assert.equal(sources.filter(source => source.purpose === 'transfer').length, 60)
