...
export async function seedPreA1PortfolioScenes(db: PrismaClient) {
    ...
    const rowsBySlug = new Map(existing.flatMap(scene => scene.revisions[0] ? [[scene.slug, scene.revisions[0]] as const] : []))
    for (const source of sources) {
        if (!rowsBySlug.has(source.slug)) rowsBySlug.set(source.slug, await platform.draft('seed:pre-a1-portfolio/1', source))
    }
    const rows = sources.map(source => rowsBySlug.get(source.slug)!)
