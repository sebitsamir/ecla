...
export async function exportLearningData(db: PrismaClient, user: User) {
    const [mastery, experienceProgress, vocabularyProgress, streaks, missions, attempts, events, memories, sceneVisits, assessments, plans] = await Promise.all([
    ...
    return { contract: 'ecla.privacy-export/1', exportedAt: new Date().toISOString(), account: user, mastery, experienceProgress, vocabularyProgress, streaks, missions, attempts, events, memories, sceneVisits, assessments, plans }
}
